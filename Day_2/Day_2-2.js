//Run in Terminal by navigating to the folder and using "node <FILE NAME.js>"
...
function getData() { //Filters each entry in the input array
    for (i = 0; i < input.length; i++) {
        paramArray = input[i].split(" ")
        minMaxArray = paramArray[0].split("-")

        min = minMaxArray[0]
        max = minMaxArray[1]
        param = paramArray[1].charAt(0)
        code = paramArray[2]

        //output from array[i] goes into checkCode()
        console.log("" + min + " " + max + " " + param + " " + code)
        checkCode(min, max, param, code) 
    } 
}

function checkCode(min, max, param, code) {  //Takes input[i] and counts number of param letter in code and validates it
    count = 0
    code = "x" + code
    console.log("" + code)

   // for (k = 0; k < code.length; k++) { //This loop counts number of times param is valid
        //min becomes pos1, max becomes pos2
        if ((param == code[min] && param != code[max]) || (param != code[min] && param == code[max])) { //check min and max == pos1 or pos1
            count = "Valid"
            console.log("param = min " + param + " = " + code[min])
            console.log("param = max " + param + " = " + code[max])
            console.log("" + count)
        } else {
            count = "Not Valid"
        }
   // } 
    if (count == "Valid") { //if count >1, then it fails
        codeCount = codeCount + 1
        console.log("Valid! ")
    } 
    console.log("Final Tally " + codeCount) //Keeps running tally of valid codes
}
